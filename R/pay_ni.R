...
pay_ni <- function(weekly_pay,
  ...
  #'
  #' @param weekly_pay Weekly pay in pounds
  #' @param standard_bracket_lower The lower pay threshold for the standard bracket
  #' @param standard_bracket_higher The higher pay threshold for the standard bracket
  ...
  weekly_ni <- dplyr::case_when(
    weekly_pay < standard_bracket_lower ~0,

    (weekly_pay > standard_bracket_lower &
       weekly_pay < standard_bracket_higher)

    ~(weekly_pay - standard_bracket_lower) * standard_ni_rate,

    weekly_pay > standard_bracket_higher ~((
      standard_bracket_higher - standard_bracket_lower) *
        standard_ni_rate) +
      ((weekly_pay-standard_bracket_higher) * upper_ni_rate))
